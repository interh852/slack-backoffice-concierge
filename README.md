...
Gmail から月次日報レポートを自動抽出し、Google ドライブの適切なフォルダへ保存します。

- **自動抽出**: Gmail をスキャンし、「月次日報レポート」を含むメールから添付ファイルを抽出します。
- **PDFファイルの識別**: 添付ファイルが PDF (`application/pdf`) であるか、または拡張子が `.pdf` のファイル（`application/octet-stream` 等）を対象とします。
- **サイト別自動仕分け**:
  - メールの件名の先頭（スペース区切り）をサイト名（ID）として認識します。
  - 設定用スプレッドシートの `monthly_daily` シートにあるマッピング情報に基づき、サイト名（ID）を実際のフォルダ名に変換して仕分けます。
- **保存構造**: 指定されたルートフォルダ配下の `{フォルダ名}/Daily Summary/` フォルダへ格納されます。
- **重複防止**: 処理済みのメールには自動的に `Report_Archived` ラベルが付与され、重複処理が防止されます。
...
| `WF_BANK_REPORT_DRIVE`         | 月次レポートのアーカイブ先となるルートフォルダの ID           |

### 4. トリガーの設定（月次レポートアーカイブ用）

月次レポートを自動でアーカイブするために、以下のトリガーを手動で設定してください。

- **実行する関数**: `archiveMonthlyReports`
- **イベントのソース**: `時間主導型`
- **トリガーのタイプ**: `日付ベースのタイマー`
- **時刻**: `午前 0 時 〜 1 時`（24時間おきに実行されるように設定）
...
月次レポートアーカイブ時のサイト名変換ルールを設定します。

- **A 列**: メールの件名から抽出されるサイト ID（例: `nikaho1`）
- **B 列**: Google Drive 上の実際のフォルダ名（例: `にかほ1`）
