...
- **自動抽出**: Gmail をスキャンし、「月次日報レポート」を含むメールから添付の PDF を取得します。
- **サイト別自動仕分け**: メールの件名の先頭（スペース区切り）をサイト名として認識し、サイトごとのフォルダに自動で保存します。
- **保存構造**: 指定されたルートフォルダ配下の `{サイト名}/Daily Summary/` フォルダへ格納されます。
