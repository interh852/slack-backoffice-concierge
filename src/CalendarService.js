...
CalendarService.prototype.getCommuteSummary = function (startDate, endDate) {
  ...
  events.forEach(function (event) {
    if (event.getTitle().indexOf(keyword) !== -1) {
      ...
      commuteDates[dateString] = (date.getMonth() + 1) + '/' + date.getDate();
    }
  });

  var sortedKeys = Object.keys(commuteDates).sort();
  var sortedDates = sortedKeys.map(function(key) {
    return commuteDates[key];
  });
